...
**Animation data is exposed NATIVELY — don't FFI it** (v9.95). `p:get_anim_state()` returns `eye_yaw / abs_yaw / anim_duck_amount / speed_as_portion_of_run_top_speed / walk_run_transition / aim_yaw_min / aim_yaw_max` (confirmed in chernobl `:3635`, frostlive, externalapaha), and `p:get_anim_overlay(i)` returns `weight / playback_rate / cycle / prev_cycle / weight_delta_rate / sequence` per animation layer. CS:GO layer indices: 3 ADJUST, 4 JUMP_OR_FALL, 6 MOVEMENT_MOVE, 7 STRAFECHANGE, 8 WHOLE_BODY, 12 LEAN. The layers are authored SERVER-SIDE against the enemy's REAL yaw, so their **per-tick deltas** carry side information the fake yaw does not mask — this is what commercial "server-side computed resolver" marketing actually refers to (it is client-side, not remote; verified by reading `unskilled_16.lua`). Read DELTAS, never absolutes: the absolute `m_flPoseParameter[11]` body-yaw mapping is a proven dead-end on this build ([[project_pose_param_deadend]]), a sign-of-change needs no calibration. Solver v9.95 `anim_side_update` / `anim_side_get` / `anim_pol_feedback` implement this with **learned** per-signal polarity (each confirmed hit scores the live signal; a backwards convention self-flips after 6+ samples). Gate any new use to fresh simulation ticks (`m_flSimulationTime` changed) — a choked tick has zero deltas.
...
## Sel01-Config layout (`sel01_config.lua`, v4.2, ~3550 lines)
...
| Movement / Visuals / QoL / Info UI | Peek Boost hotkey, **AI Peek v4.1** (`AIP` table: enable / trigger / key / distance / confirm ticks / max exposure / cooldown / retreat mode / DT wait / DT teleport / pause-on-manual / hitboxes / HC / safe-points / weapon filter / draw / dev), visual switches, clantag, Info buttons incl. **📋 Copy Last Logs** |
...
- **V4.2 (2026-09-07, bindable switches)**: user rule — **no script hotkeys, no trigger combos**. NL can bind ANY switch to a key (right-click, hold or toggle), so every "key" feature is a plain switch: AI Peek `AIP.active`, Manual Left/Right/Forward, Force defensive, Peek Boost. Also fixed the v4.1 AI Peek instant-reset: the "manual movement" pause read `cmd.forwardmove/sidemove`, which NL writes itself (auto-stop / peek assist); it reads `cmd.in_forward/in_back/in_moveleft/in_moveright` now.
- **V4.1 (2026-09-07, AI Peek rewrite + first-dump fixes)**: AI Peek = aiPeek_61044 algorithm. Anchor where you stand (ground + speed < 15), every tick: candidates perpendicular to `entity.get_threat()` (one per side at `Peek distance`, or a 7-step ladder on the locked side, `utils.trace_line` brush-only at eye height, plus the anchor itself as candidate #0 while holding) → `utils.trace_bullet(lp, candidate_eye, hitbox, lp)` against head(+2z)/chest(5)/pelvis(2) of every alive enemy (locked enemy only after commit), need `max(NL Min. Damage - 5, 5)` capped at enemy hp; enemy defensive (sim time went backwards) = body-only for 12 ticks. Confirm N ticks → lock side/enemy 25 ticks → P-move with the angelwings reverse-thrust quick-stop inside 5u → retreat on `events.aim_fire` (or when no shot exists), exposure timeout, cooldown, `rage.exploit:force_teleport()` once per retreat when charged. Gates: hold key / always-on, manual movement pauses + re-anchors, weapon type (no knife/nade/c4), clip, `m_flNextPrimaryAttack`, optional DT-charge wait (default OFF per user), airborne → hold. HC override default 0 = keep NL. Elements in the `AIP` table; state in `ai_peek`; `[AIPEEK]` line + `AIPEEK` timeline entries in the copy dump; keybinds panel shows the phase; `render.circle_3d` peek point / target. Also: Crouching + Crouch-Move own rows in defaults + presets, `_nl_eff` (override-aware) for the dump's `[NL]` line, STATE timeline throttle (0.3s dwell), hints for hits during active defensive / on Global rows / one-tap lobbies.
...
- **Plant cancelled by our own `slot5`** (v1.8). BOTH `c4_arming` (`lp:get_weapon().m_bStartedArming`) and `active_is_c4` are build-dependent reads that silently return false; with both blind, `equip_c4` re-exec'd `slot5` every 0.4s DURING the arm and cancelled it — user saw "+attack only tapped, never held". Fixes: CC4-entity fallback for the arming netvar, `arm_read_ok` flag (abort-by-netvar detection only trusted when the netvar has ever read true, else the `bomb_abortplant` event), `arming = live netvar OR bomb_beginplant event`, and a hard `attack_since` lock — once +attack has been held 0.3s NO weapon switch can fire; recovery runs through the reposition/abort branches which clear it.
- **Nav ping-pong right before a corner** (v1.9, same flaw exists in WalkBot's `nav_dir`). `nav_astar` returns the START area as node 1; standing at that area's edge its center is 200-300u BEHIND you, and the old pop loop only dropped nodes <130u → the bot walked BACK to its own area center, popped it, walked forward again, and the 1.5s re-path restarted the loop. `nav_dir(lp, lo, gx, gy, now)` now: (1) drops the node of the area we stand in (`path[1].id == sa.id`), (2) skips a leading node that a LATER node beats on straight-line distance, gated on `probe_clear` toward that later node so corners are never cut through walls, (3) the usual 130u reach pop. Tracks `state.nav_node_d`.
- **Debug logging** (v1.9, WalkBot pattern): `state.logs` ring (160) + `pb_log(key, throttle, fmt, ...)` — ALWAYS buffered, printed live to console only when `Debug Info` is on, throttled per key. `Dump PlantBot Logs (console)` button prints the ring + a header (map/phase/nav/aborts/wedges/repaths). Instrumented: phase transitions, run reset, A* re-path/fail/node-advance/start-area churn (`start=NN*` marks a churn), avoid/boxed/duck-pass/strafe/WEDGE/recovery with hspeed+coords, plant arming/abort/"attack did nothing"/timeout, and a **2s heartbeat with the goal-distance TREND** (`closing / flat / AWAY +N`) — the fastest way to spot a route problem. `pb_log` pushes to the ring on EVERY call, so self-throttle any per-tick call site (the heartbeat gates on `state.hb_t`) or the buffer floods.
- **Bunny-hop on open stretches** (v1.9): own switch, default ON, independent of `Allow Jumping` (which still governs obstacle/step-up/wedge jumps). `bhop_ok` needs a 360u clear lane, both shoulders clear at ±35°, heading steady 0.4s, next nav node >220u, goal >400u, `block == "clear"`, not stuck/ducking. NEVER in the PLANT reposition path — a jump there cancels the arm.
